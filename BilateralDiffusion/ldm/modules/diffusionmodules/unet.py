...
class ResBlock(TimestepBlock):
    ...
    def forward(self, x, emb, depth=None):
        """
...
class UNetModel(nn.Module):
    ...
    def __init__(
            ...
            for _ in range(num_res_blocks):
                layers = [
                    ResBlock(
                        ...
                        use_dcnn=bool('input' in conf.get('dcnn_mode',[]))
                    )
                ]
        ...
        self.middle_block = TimestepEmbedSequential(
            ResBlock(
                ...
                use_checkpoint=use_checkpoint,
                use_scale_shift_norm=use_scale_shift_norm,
                use_dcnn=bool('middle' in conf.get('dcnn_mode',[]))
            ),
            AttentionBlock(
                ch,
                use_checkpoint=use_checkpoint,
            ...
            for i in range(num_res_blocks + 1):
                ...
                layers = [
                    ResBlock(
                        ...
                        use_checkpoint=use_checkpoint,
                        use_scale_shift_norm=use_scale_shift_norm,
                        use_dcnn=bool('output' in conf.get('dcnn_mode',[]))
                    )
                ]
    ...
    def forward(self, x, timesteps, y=None, gt=None, **kwargs):
        ...
        for module in self.input_blocks:
            h = module(h, emb, depth=dps)
            hs.append(h)

        h = self.middle_block(h, emb, depth=dps)
