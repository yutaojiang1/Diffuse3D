...
def extrapolate(global_mesh,
    ...
    if config.get('mix_mode') == 'pmix':
        t_image = torch.FloatTensor(image.transpose(2, 0, 1)).unsqueeze(0).to(device)
        depth_tensor = torch.from_numpy(depth).float().unsqueeze(0).unsqueeze(0).to(device)
        rgb_output = rgb_feat_model.forward_inpaint(t_mask, t_context, t_image, all_anchor, unit_length=128,
                                            cuda=device,global_only=True,depth=depth_tensor)
    else:
        depth_tensor = torch.from_numpy(depth_output).float().unsqueeze(0).unsqueeze(0).to(device)
        rgb_output = rgb_feat_model.forward_3P(t_mask, t_context, t_rgb, t_update_edge, unit_length=128,
                                            cuda=device,depth=depth_tensor)
