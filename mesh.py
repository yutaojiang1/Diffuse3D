...
def DL_inpaint_edge(mesh,
    ...
    for edge_id, (context_cc, mask_cc, erode_context_cc, extend_context_cc, edge_cc) in enumerate(zip(context_ccs, mask_ccs, erode_context_ccs, extend_context_ccs, edge_ccs)):
        ...
        resize_rgb_dict['resize_mask'] = resize_mask

        with torch.no_grad():
            if config.get('mix_mode') == 'pmix':
                depth_in = depth_dict['output'].copy()
                rgb_output = rgb_model.forward_inpaint(resize_rgb_dict['resize_mask'].clone(),
                                            resize_rgb_dict['context'].clone(),
                                            image.clone(),
                                            union_size,
                                            unit_length=128,
                                            cuda=device,
                                            depth=depth,
                                            depth_patch=depth_in[union_size['x_min']:union_size['x_max'], union_size['y_min']:union_size['y_max']])
            else:
                depth_in = depth_dict['output'].copy()
                # depth_in[(depth_dict['mask']+depth_dict['context']) == 0] = 1e4
                rgb_output = rgb_model.forward_3P(resize_rgb_dict['resize_mask'],
                                            resize_rgb_dict['context'],
                                            resize_rgb_dict['rgb'],
                                            resize_rgb_dict['edge'],
                                            unit_length=128,
                                            cuda=device,
                                            depth=depth_in[union_size['x_min']:union_size['x_max'], union_size['y_min']:union_size['y_max']])
            rgb_output = rgb_output.cpu()
            if config.get('gray_image') is True:
                rgb_output = rgb_output.mean(1, keepdim=True).repeat((1,3,1,1))
            rgb_output = rgb_output.cpu()
        resize_rgb_dict['raw_output'] = rgb_output
...
class Canvas_view():
    ...
    def set_trans_mtx(self,trans_mtx):
        self.tr.matrix = trans_mtx.transpose()
